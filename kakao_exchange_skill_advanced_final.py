...
def _arrow_and_delta(prev: float | None, cur: float):
    ...
    if abs(delta) < 1e-9:
        return "", "0"
    return ("▲" if delta > 0 else "▼"), f"{abs(delta):,.2f}"
...
def _fmt_line(label: str, value: str, arrow: str = "", delta: str = ""):
    tail = ""
    if arrow and delta:
        tail = f" {arrow}{delta}"
    return f"{label} {value}{tail}".strip()
...
def get_fx():
    ...
    data = [
        {"key": "USD", "label": "🇺🇸 USD(달러)", "value": krw_per_usd},
        {"key": "JPY100", "label": "🇯🇵 JPY100(엔)", "value": (krw_per_usd / jpy_per_usd) * 100.0},
        {"key": "EUR", "label": "🇪🇺 EUR(유로)", "value": krw_per(eur_per_usd)},
        {"key": "CNY", "label": "🇨🇳 CNY(위안)", "value": krw_per(cny_per_usd)},
        {"key": "GBP", "label": "🇬🇧 GBP(파운드)", "value": krw_per(gbp_per_usd)},
    ]
...
def _scrape_value_and_change(text: str, names):
    ...
    def find_near(keyword: str):
        # Look for: keyword ... number ... (optional arrow + number)
        # Example in page text often: 코스피 2,500.12 전일대비 +12.34
        # We'll do a simple regex window.
        ...
        if not nums:
            return None
        value = nums[0]
        # find change as second number if exists
...
def get_indices():
    ...
    for row in scraped:
        ...
        # If MK scraping fails for change, compute vs previous.
        if v is not None:
...
def exchange_rate():
    # Kakao request JSON (not strictly needed)
    ...
    fx_desc = _build_description([
        _fmt_line(row["label"], row["rate"], row["arrow"], row["delta"]) for row in fx
    ])
    idx_desc = _build_description([
        _fmt_line(row["label"], row["value"], row["arrow"], row["delta"]) for row in idxs
    ])
    comm_desc = _build_description([
        _fmt_line(row["label"], row["value"], row["arrow"], row["delta"]) for row in comm
    ])
    crypto_desc = _build_description([
        _fmt_line(row["label"], row["value"], row["arrow"], row["delta"]) for row in crypto
    ])
